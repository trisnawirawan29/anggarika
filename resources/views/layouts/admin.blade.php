...
<div class="app-wrapper">
    ...
    <aside class="app-sidebar bg-dark shadow" data-bs-theme="dark">
        ...
        <div class="sidebar-wrapper"><nav>
            <div class="user-panel d-flex align-items-center"><span class="avatar avatar-lg">@if(auth()->user()->avatar)<img src="{{ asset('storage/'.auth()->user()->avatar) }}" alt="Avatar">@else{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}@endif</span><div class="info"><a href="{{ route('profile') }}" class="d-block text-white">{{ auth()->user()->name }}</a><small>{{ ucfirst(auth()->user()->roleSlug()) }}</small></div></div>
            ...
            @if (auth()->user()->isAdmin())
                <div class="nav-header mt-3">ADMINISTRASI</div>
                <ul class="nav sidebar-menu flex-column" role="menu">
                    <li class="nav-item {{ request()->routeIs('admin.*') ? 'menu-open' : '' }}">
                        <a href="#" class="nav-link {{ request()->routeIs('admin.*') ? 'active' : '' }}" data-lte-toggle="treeview" role="button">
                            <i class="nav-icon fas fa-toolbox"></i><p>Administrasi<i class="nav-arrow fas fa-angle-right"></i></p>
                        </a>
                        <ul class="nav nav-treeview">
                            <li class="nav-item"><a href="{{ route('admin.users') }}" class="nav-link {{ request()->routeIs('admin.users*') ? 'active' : '' }}"><i class="nav-icon fas fa-users-cog"></i><p>Pengguna</p></a></li>
                            <li class="nav-item"><a href="{{ route('admin.settings') }}" class="nav-link {{ request()->routeIs('admin.settings*') ? 'active' : '' }}"><i class="nav-icon fas fa-sliders"></i><p>Pengaturan Aplikasi</p></a></li>
                            <li class="nav-item"><a href="{{ route('admin.audit-logs') }}" class="nav-link {{ request()->routeIs('admin.audit-logs*') ? 'active' : '' }}"><i class="nav-icon fas fa-clock-rotate-left"></i><p>Audit Log</p></a></li>
                        </ul>
                    </li>
                </ul>
            @endif
            <div class="nav-header mt-3">SISTEM</div>
            <ul class="nav sidebar-menu flex-column" role="menu">
