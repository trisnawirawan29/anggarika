...
@section('page-subtitle', 'Kelola data pengguna dan tiga role aplikasi.')
...
@section('content')
    <div class="content-card">
        ...
        <form method="GET" class="row g-2 mb-4">
            ...
            <div class="col-md-3"><select name="role" class="form-select"><option value="">Semua role</option><option value="superadmin" @selected(request('role') === 'superadmin')>Superadmin</option><option value="admin" @selected(request('role') === 'admin')>Admin</option><option value="user" @selected(request('role') === 'user')>User</option></select></div>
        ...
        <div class="table-responsive"><table class="table align-middle data-table" data-data-table><thead><tr><th>NAMA</th><th>EMAIL</th><th>ROLE</th><th>AKSI</th></tr></thead><tbody>
            @forelse ($users as $user)
                <tr><td class="fw-semibold">{{ $user->name }}</td><td class="text-muted">{{ $user->email }}</td><td><span class="status {{ $user->role === 'superadmin' ? 'status-danger' : ($user->role === 'admin' ? 'status-warning' : 'status-success') }}">{{ ucfirst($user->role) }}</span></td><td><div class="d-flex gap-2"><a href="{{ route('admin.users.edit', $user) }}" class="btn btn-sm btn-light"><i class="fas fa-pen"></i></a>@unless($user->is(auth()->user()))<form method="POST" action="{{ route('admin.users.destroy', $user) }}" data-confirm="Hapus pengguna ini? Data yang dihapus tidak dapat dikembalikan.">@csrf @method('DELETE')<button class="btn btn-sm btn-light text-danger"><i class="fas fa-trash"></i></button></form>@endunless</div></td></tr>
            @empty
